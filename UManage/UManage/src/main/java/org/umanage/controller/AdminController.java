...
public class AdminController {
	...
	public ResponseEntity<?> newUser(@RequestBody @Valid AppUser newUser)
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(modelMapper.map(savedUser, AppUserResponse.class));
	}
	...
	public List<AppUserResponse> getAllUsers() {
		return userService.getAllUsers()
				.stream()
				.map(user -> modelMapper.map(user, AppUserResponse.class)).toList();
	}
